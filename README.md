...
本仓库当前只完成用于后续研究的数据生成与无泄漏划分基线，不包含轨迹预测、规划训练或无人机控制模型。
...
实验 1 脚本会在 `outputs/experiment_01/` 生成单场景数据。实验 1.1 脚本会在 `outputs/dataset_v2/` 生成 `train.npz`、`validation.npz`、`test.npz`、manifest、日志和 figures。`outputs/` 是自动生成目录，不纳入 Git 版本控制；仓库只保留 `outputs/.gitkeep` 以维持目录结构。
...
- 当前尚未建立任何轨迹预测基线，进入模型实验前必须坚持 scene/episode 级评估。
